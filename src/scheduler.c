...
void initialize_Scheduler(Scheduler* scheduler, Process* processes, int number_of_processes, int q)
{
    ...
    ProcessHeap* high_queue = create_heap(2*q, number_of_processes);
...
{
    // Si hay un proceso en estado RUNNING, actualizar su estado según el siguiente orden:
    //  1) Alcanzó su deadline
    //  2) Terminó su CPU burst
    //  3) Su quantum se acabó
    //  4) Si ocurre un evento que involucra a un proceso distinto al que está en la CPU, entonces el proceso en ejecución debe abandonar la CPU
    ...
    for (size_t i = 0; i < scheduler->process_count; i++)
    {
        ...
        {
            // 1) Alcanzó su deadline
            if (process.deadline_time <= scheduler->current_tick && process.bursts_remaining > 0)
            {
                process.state = DEAD;
                continue;
            }
            ...
            // 2) Terminó su CPU burst
            if (process.time_spent_on_burst >= process.burst_duration)
            {
                process.bursts_remaining -= 1;
            }

            // 3) Su quantum se acabó
            if (process.time_spent_on_quantum == scheduler->high_queue->quantum)
            {
            ...
            // 4) Sacar proceso por evento (no implementado)

            // 5) Continua ejecutando con normalidad
            process.time_spent_on_burst += 1;
            process.time_spent_on_quantum += 1;
        }
    }
}
...
{
    // Actualizar las prioridades de todos los procesos según la fórmula dada.
}
