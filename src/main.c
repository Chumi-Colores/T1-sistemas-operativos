...
void run_simulation(EventController* event_controller, Scheduler* scheduler)
{
    ...
    while (scheduler->pending_processes > 0 || event_controller->pending_events > 0)
    {
        printf("pending processes: %d, pending events: %d\n", scheduler->pending_processes, event_controller->pending_events);
